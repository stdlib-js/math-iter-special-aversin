...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterAversin, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object', function test( t ) {
	...
	it = iterAversin( uniform( 0.0, 2.0 ) );
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	values = [ 0.0, 0.5, 1.0 ];
	it = iterAversin( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which computes the inverse versed sine of each iterated value', function test( t ) {
	var expected;
	var it;
	var N;
	var x;
	...
	N = 101;
	it = iterAversin( linspace( 0.0, 2.0, N ) );
	t.equal( it.next.length, 0, 'has zero arity' );

	x = linspace( 0.0, 2.0, N );
	for ( i = 0; i < N; i++ ) {
		r = it.next();
		expected = aversin( x.next().value );
...
tape( 'the function returns an iterator protocol-compliant object which computes the inverse versed sine of each iterated value (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	values = [ 1.0, 0.5, 0.0, 1.5 ];
	expected = [
		{
			'value': aversin( values[ 0 ] ),
			'done': false
		},
		{
			'value': aversin( values[ 1 ] ),
			'done': false
		},
		{
			'value': aversin( values[ 2 ] ),
			'done': false
		},
		{
			'value': aversin( values[ 3 ] ),
			'done': true
		}
	];
